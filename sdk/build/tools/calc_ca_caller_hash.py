...
def calc_app_caller_hash(app_pkg_name, app_modulus, app_public_exponent):
    hex_modulus = binascii.a2b_hex(app_modulus)
    hex_exponent = binascii.a2b_hex(app_public_exponent)
    ...
    c_str = create_string_buffer((app_pkg_name).encode('utf-8'), \
        len(app_pkg_name) + len(hex_pub_key))
...
def handle_app_item_hash(item):
    ...
    for attr in item.attrib:
        ...
        if attr == "pkg_name":
            ...
            raise RuntimeError("invalid item attr", attr)
    caller_hash = calc_app_caller_hash(pkg_name, modulue, exponent)
    logging.info("pkg_name: %s, modulue:%s, exponent: %s", pkg_name, modulue, exponent)
    print_hash(caller_hash)


def handle_app(child):
    for item in child:
        check_app_ca_item(item)
        handle_app_item_hash(item)
