...
#use java parse xml
XML2TLV_JAR_VALUE = 0 << XML2TLV_PARSE_TOOL_INDEX
#use python parse xml
XML2TLV_PY_VALUE = 1 << XML2TLV_PARSE_TOOL_INDEX
logging.basicConfig(level=logging.INFO)


def get_policy_version(config_file):
    ...
    if os.path.isfile("./xml2tlv.jar"):
        policy_ver = base_policy_ver | XML2TLV_JAR_VALUE
    else:
        policy_ver = base_policy_ver | XML2TLV_PY_VALUE
    return policy_ver
...
def gen_rsa_signature(sign_conf_alg, config_buf, input_path_gen, output_file):
    if sign_conf_alg == "RSA_PSS":
        ...
        try:
            ...
            raise RuntimeError
        logging.info("Sign Config with PSS Success")
    else:
...
def convert_xml2tlv(xml_file, tlv_file, input_path, config_file):
    policy_ver = get_policy_version(config_file)
    if (policy_ver & (1 << XML2TLV_PARSE_TOOL_INDEX)) == XML2TLV_JAR_VALUE:
        cmd = ["java", "-jar", "xml2tlv.jar", xml_file, tlv_file]
        run_cmd(cmd)
        if os.path.isfile(tlv_file):
            logging.info('convert xml to tlv success')
    elif (policy_ver & (1 << XML2TLV_PARSE_TOOL_INDEX)) == XML2TLV_PY_VALUE:
        csv_dir = os.path.realpath(os.path.join(os.getcwd(), './'))
...
class Configuration:
    ...
    def __init__(self, file_name):
        ...
        self.policy = parser.get("signConfigPrivateCfg", "configPolicy")
    
    def check_cfg_format(self):
...
def gen_config_header(sign_conf_alg, config_content_size, ta_cert_size, config_cert_size, config_file):
    config_hd_len = 44
    if sign_conf_alg == "ECDSA":
    ...
    config_header = pkg_config_header(config_hd_len, 0xABCDABCD, \
            CONFIG_VERSION, get_policy_version(config_file), config_context_size, \
            ta_cert_size, config_content_size, config_sign_size, \
            config_cert_size)
    return config_header, final_sign_size


def get_tlv_buffer(input_path, config_file):
    #convert xml to tlv
    ...
    return tlv_config_buf, config_content_size
...
def gen_config_section(input_path, cert_path, config_section):
    ...
    tlv_config_buf, config_content_size = get_tlv_buffer(input_path, config_file)
    ...
    config_header, final_sign_size = gen_config_header(sign_conf_alg, config_content_size, ta_cert_size, \
                                    config_cert_size, config_file)
