...
def check_cfg_integer_format(intput_str):
    if intput_str != "":
        if integer_check(intput_str):
            return 1
    return 0
...
def gen_header(content_len, cfg):
    ...
    return struct.pack('IHHII', MAGIC1, MAGIC2, cfg.ta_version, content_len, \
            key_version)
...
def gen_signature(cfg, uuid_str, data_for_sign, key_info_data, temp_path):
    ...
    gen_ta_signature(cfg, uuid_str, data_for_sign, data_for_sign_path, \
        hash_file_path, signature_path, cfg.out_path, key_info_data)
...
def pack_sec_img(cfg, manifest_info, temp_path):
    ''' pack sec img: header || key || signature || raw_data '''
    signature_path = os.path.join(temp_path, "signature.bin")
    key_data_path, raw_data_path = get_data_path(cfg, temp_path)
